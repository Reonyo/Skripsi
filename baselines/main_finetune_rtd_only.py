"""
Baseline Finetune: RTD-Only
Fine-tune RTD-only baseline on GLUE tasks.
"""
import sys
import os
...
def main():
    ...
    print("="*60)
    print("BASELINE FINETUNE: RTD-Only")
    print("="*60)
    
    # Load configs
    model_cfg = load_yaml("baselines/configs/rtd_only.yaml")
    finetune_cfg = load_yaml("baselines/configs/finetune_rtd_only.yaml")
    ...
    print("Loading pretrained RTD-only weights...")
    pretrained_path = checkpoint_cfg.get(
        "pretrained_path", 
        "outputs/baselines/rtd_only/checkpoint_step_100000.pt"
    )
    ...
    # Output directory
    output_dir = Path(f"outputs/baselines/rtd_only_finetune/{task_name}")
    ...
    print(f"\n[OK] RTD-only baseline finetuning complete for {task_name}")
